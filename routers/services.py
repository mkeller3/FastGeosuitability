...
@router.post("/line_suitability/", tags=["Services"])
async def line_suitability(info: models.LineSuitability, request: Request):
    """
    Method to enrich map with suitability metrics
    ...
    async with pool.acquire() as con:
        ...
        for index, line in enumerate(info.geojson_collection.features):
            ...
            geojson = {
                ...
                "coordinates": json.loads(json.dumps(line.geometry.coordinates))
            }

            if info.return_geometry:                
                query += f""" ST_AsGeoJSON(ST_Transform(ST_Buffer(ST_Transform(ST_GeomFromGeoJSON('{json.dumps(geojson)}'),3857), {info.buffer_in_kilometers*1000}),4326)) as geometry"""
            else:
                query += f"""null as geometry"""
                ...
                for variable in info.variables:
                    ...
                    if 'Polygon' in geometry_type['geometry_type']:
                        query = f"""
                            ...
                            FROM {variable.table} as a, ST_Transform(ST_Buffer(ST_Transform(ST_GeomFromGeoJSON('{json.dumps(geojson)}'),3857), {info.buffer_in_kilometers*1000}),4326) as b
                            WHERE ST_Intersects(b, a.geom)
                        """
                    else:
                        query = f"""
